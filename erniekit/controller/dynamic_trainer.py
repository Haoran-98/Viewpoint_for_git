...
class BaseDynamicTrainer(object):
    ...
    def __init__(self, params, data_set_reader, model_class):
        ...
        if 'output_path' in self.params.keys() and self.params["output_path"]:
            self.save_checkpoints_path = os.path.join(self.params["output_path"], "save_checkpoints" + current_time)
            self.save_inference_model_path = os.path.join(self.params["output_path"], "save_inference_model" + current_time)
        else:
            self.save_checkpoints_path = "./output/save_checkpoints" + current_time + "/"
            self.save_inference_model_path = "./output/save_inference_model" + current_time + "/"
